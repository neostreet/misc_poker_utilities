...
static char usage[] =
"usage: fzero_bet_pct (-debug) (-verbose) player_name filename\n";
...
int main(int argc,char **argv)
{
  ...
  if (zero_bet_possibilities)
    dwork = (double)zero_bet_count / (double)zero_bet_possibilities;
  else
    dwork = (double)0;
  ...
  if (!bVerbose)
    ...
    printf("%lf (%d %d) %s\n",dwork,
      zero_bet_count,zero_bet_possibilities,save_dir);
