...
static char usage[] =
"usage: session_moving_winpct (-no_sort) (-ascending) subset_size filename\n";
...
int main(int argc,char **argv)
{
  ...
  num_winpcts = set_size - subset_size + 1;
  ...
  for (n = 0; n < num_winpcts; n++) {
    ...
    winpct = (double)wins / (double)subset_size;
    session_info[n].winpct = winpct;
    session_info[n].end_date = session_info[n+subset_size-1].start_date;
    session_info[n].wins = wins;
    sort_ixs[n] = n;
  }
...
int elem_compare(const void *elem1,const void *elem2)
{
  ...
  if (session_info[ix1].winpct == session_info[ix2].winpct)
  ...
  if (bAscending) {
    ...
    if (session_info[ix2].winpct < session_info[ix1].winpct)
      return -1;
    else
      return 1;
  }
