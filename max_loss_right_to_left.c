...
static char usage[] =
"usage: max_loss_right_to_left (-debug) (-verbose) (-no_sort) (-sort_by_avg) (-date_last) filename\n";
...
int main(int argc,char **argv)
{
  ...
  for (m = num_sessions - 1; m >= 0; m--) {
    ...
    for (n = m; n >= 0; n--) {
      work = session_info[n].starting_amount - session_info[m].ending_amount;
    ...
    if (max_loss) {
      session_info[m].num_loss_sessions = m - max_loss_ix + 1;
      ...
      session_info[m].loss_start_date = session_info[max_loss_ix].loss_end_date;
    }
  }
  ...
  for (n = 0; n < num_losses; n++) {
    printf(fmt1,
      session_info[sort_ixs[n]].ending_amount +
        session_info[sort_ixs[n]].loss_amount,
      session_info[sort_ixs[n]].ending_ix -
  ...
  struct session_info_struct *session_info)
{
  ...
  if (!bDateLast) {
    ...
    session_info->loss_end_date = cvt_date(line);
    ...
    session_info->loss_end_date = cvt_date(&line[n+2]);
