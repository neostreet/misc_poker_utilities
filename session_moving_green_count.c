...
static char usage[] =
"usage: session_moving_green_count (-no_sort) (-ascending) subset_size filename\n";
...
int main(int argc,char **argv)
{
  ...
  num_green_counts = set_size - subset_size + 1;
  ...
  if ((sort_ixs = (int *)malloc(
    num_green_counts * sizeof (int))) == NULL) {
    printf(malloc_failed2,num_green_counts);
    fclose(fptr);
    return 6;
  }
  ...
  for (n = 0; n < set_size; n++) {
    if (session_info[n].delta > 0)
      session_info[n].is_green = 1;
    else
      session_info[n].is_green = 0;
  }

  for (n = 0; n < num_green_counts; n++) {
    green_count = 0;

    for (m = 0; m < subset_size; m++)
      green_count += session_info[n+m].is_green;

    session_info[n].green_count = green_count;
    session_info[n].end_date = session_info[n+subset_size-1].start_date;
    sort_ixs[n] = n;
  }

  if (!bNoSort)
    qsort(sort_ixs,num_green_counts,sizeof (int),elem_compare);

  for (n = 0; n < num_green_counts; n++) {
    printf("%10d ",session_info[sort_ixs[n]].green_count);
...
int elem_compare(const void *elem1,const void *elem2)
{
  ...
  if (session_info[ix1].green_count == session_info[ix2].green_count)
    return session_info[ix2].end_date - session_info[ix1].end_date;

  if (bAscending)
    return session_info[ix1].green_count - session_info[ix2].green_count;
  else
    return session_info[ix2].green_count - session_info[ix1].green_count;
}
