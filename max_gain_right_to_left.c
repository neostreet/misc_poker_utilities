...
static char usage[] =
"usage: max_gain_right_to_left (-debug) (-verbose) (-no_sort) (-sort_by_avg) (-date_last) filename\n";
...
int main(int argc,char **argv)
{
  ...
  for (m = num_sessions - 1; m >= 0; m--) {
    ...
    if (max_gain) {
      session_info[m].num_gain_sessions = m - max_gain_ix + 1;
      ...
      session_info[m].gain_start_date = session_info[max_gain_ix].gain_end_date;
    }
  }
  ...
  struct session_info_struct *session_info)
{
  ...
  if (!bDateLast) {
    ...
    session_info->gain_end_date = cvt_date(&line[n+2]);
