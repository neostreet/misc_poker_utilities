...
static char usage[] =
"usage: session_moving_sum2 (-no_sort) (-ascending) (-absolute_value)\n"
"  (-skip_interim) (-terse) (-gesum) (-true_false) (-delta_first)\n"
"  (-outer_sort_by_winning_sessions) (-second_delta) (-homogenous)"
"  (-count_nonlosing) lo_subset_size hi_subset_size filename\n";
...
int main(int argc,char **argv)
{
  ...
  sscanf(argv[curr_arg++],"%d",&lo_subset_size);
  sscanf(argv[curr_arg++],"%d",&hi_subset_size);

  if (lo_subset_size > hi_subset_size) {
    printf("lo_subset_size (%d) > hi_subset_size (%d)\n",
      lo_subset_size,hi_subset_size);
    return 3;
  }
  ...
  if (hi_subset_size > set_size) {
    printf("hi_subset_size (%d) > set_size (%d)\n",hi_subset_size,set_size);
    fclose(fptr);
    return 5;
  }
  ...
  for (subset_size = lo_subset_size; subset_size <= hi_subset_size; subset_size++) {
    sprintf(filename,"%s.%d%s.session_moving_sum2",
      argv[curr_arg],subset_size,
    ...
    for (n = 0; n < num_sums; n++) {
      ...
      if (!bSecondDelta) {
        if (bTerse)
          ...
          fprintf(ofptr,"%10d ",session_info[sort_ixs[n]].sum);
      }
      else {
        if (bTerse) {
          fprintf(ofptr,"%10d %10d\n",session_info[sort_ixs[n]].sum,
            session_info[sort_ixs[n]].second_sum);
        }
        else {
          fprintf(ofptr,"%10d %10d ",session_info[sort_ixs[n]].sum,
            session_info[sort_ixs[n]].second_sum);
        }
      }
      ...
      if (!bTerse) {
        ...
        fprintf(ofptr,"%s ",format_date(cpt));
