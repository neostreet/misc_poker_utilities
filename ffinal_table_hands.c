...
static char usage[] =
"usage: ffinal_table_hands (-pct) (-verbose) initial_stake num_entries filename\n";
...
int main(int argc,char **argv)
{
  ...
  if (final_table_hands) {
    if (!bPct)
      printf("%d %s\n",final_table_hands,save_dir);
    else {
      pct = (double)final_table_hands / (double)hands;

      if (!bVerbose)
        printf("%lf %s\n",pct,save_dir);
      else {
        printf("%lf (%d %d) %s\n",pct,
          final_table_hands,hands,save_dir);
      }
    }
  }
