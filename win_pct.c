...
static char usage[] = "usage: win_pct (-verbose) player_name filename\n";
...
int main(int argc,char **argv)
{
  ...
  if (!total_hands)
    ...
    win_pct = (double)total_wins / (double)total_hands;

  printf("%8.6lf ",win_pct);
  printf("(%d %d)",total_wins,total_hands);
