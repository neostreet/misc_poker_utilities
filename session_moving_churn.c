...
static char usage[] =
"usage: session_moving_churn (-no_sort) (-ascending) subset_size filename\n";
...
int elem_compare(const void *elem1,const void *elem2)
{
  ...
  if (session_info[ix1].churn == session_info[ix2].churn)
  ...
  if (bAscending) {
    ...
    if (session_info[ix2].churn > session_info[ix1].churn)
      return 1;
    else
      return -1;
  }
