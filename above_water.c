...
int main(int argc,char **argv)
{
  ...
  for (m = 0; m < NUM_POKER_STYLES; m++) {
    for (n = 0; n < NUM_POKER_FLAVORS; n++) {
      ...
      bal_and_count[m][n].above_water_count = 0;
    }
  }
  ...
  for ( ; ; ) {
    ...
    if (bal_and_count[style][flavor].curr_balance > 0)
      bal_and_count[style][flavor].above_water_count++;
  }
  ...
  for (m = 0; m < NUM_POKER_STYLES; m++) {
    for (n = 0; n < NUM_POKER_FLAVORS; n++) {
      if (bal_and_count[m][n].above_water_count && (bal_and_count[m][n].above_water_count == bal_and_count[m][n].count)) {
        printf("%10d %6d   %s   %s\n",
