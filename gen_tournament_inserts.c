...
static char usage[] =
"usage: gen_tournament_inserts player_name poker_style poker_flavor tournament_filename\n"
"  big_blind_amount\n";
...
int main(int argc,char **argv)
{
  ...
  for (n = 0; n < MAX_SIT_AND_GOS; n++) {
    sprintf(outer_filename,"%c/%s.lst",letter,argv[ARG_TOURNAMENT_FILENAME]);
    ...
    sprintf(outer_filename,"%c/%s.ls0",letter,argv[ARG_TOURNAMENT_FILENAME]);
